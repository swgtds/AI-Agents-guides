...
- Project README: `managing-loan-approvals/README.md`
- Swagger / API (local): http://localhost:4111/swagger-ui (if enabled) or test via curl.
...
<CardGroup>
  <Card title="Coordinator" description="Parses intent & delegates to specialized agent" />
  <Card title="Specialists" description="Perform domain-specific analysis (credit, docs, risk, decision)" />
  <Card title="Tool" description="Programmatic entrypoint for orchestration" />
  <Card title="Workflow" description="Composable step returning structured approval context" />
</CardGroup>
...
- Mastra
  - `src/mastra/index.ts` (register agents + workflows)
  - `src/mastra/agents/` (specialist + coordinator agents)
  - `src/mastra/tools/loan-approval-coordinator-tool.ts`
  - `src/mastra/workflows/loan-approval-coordinator-workflow.ts`
- Entry
  - `src/index.ts` (app bootstrap if present)
